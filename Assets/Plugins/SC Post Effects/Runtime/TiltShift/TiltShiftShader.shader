Shader "Hidden/SC Post Effects/Tilt Shift"
{
	...
	float4 FragHorizontal(Varyings input) : SV_Target
	{
		...
		{
			float2 sampleUV = UV.xy + SmallDiscKernel[l].xy * poissonScale.xy;
	...
	float4 FragHorizontalHQ(Varyings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float3 sum = 0;
		...
		{
			float2 sampleUV = UV.xy + DiscKernel[l].xy * poissonScale.xy;
			float3 sample0 = ScreenColor(sampleUV.xy).rgb;
	...
	float4 FragRadial(Varyings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float3 sum = 0;
	...
	float4 FragRadialHQ(Varyings input) : SV_Target
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float3 sum = 0;
	...
	float4 FragBlend(Varyings input) : SV_Target
	{
		...
		float4 blurredColor = SAMPLE_TEXTURE2D(_BlurredTex, Clamp, UV);
