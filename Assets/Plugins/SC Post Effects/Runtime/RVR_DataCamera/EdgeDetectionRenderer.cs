...
namespace SCPE
{
    public sealed class EdgeDetectionRenderer : PostProcessEffectRenderer<EdgeDetection>
    {
        ...
        public override void Render(PostProcessRenderContext context)
        {
            ...
            sheet.properties.SetTexture("_ObjectsStencil", settings.objectStencil);

            sheet.properties.SetVector(ShaderParameters.FadeParams, new Vector4(settings.startFadeDistance.value, settings.endFadeDistance.value, (settings.invertFadeDistance.value) ? 1 : 0, settings.distanceFade.value ? 1 : 0));
